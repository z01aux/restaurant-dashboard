...
// Hook para gestionar pedidos OEP (con tipos importados)
// Equivalente exacto de: src/hooks/useFullDayOrders.ts
// ============================================================
...
export const useOEPOrders = () => {
  ...
  const updateOrderPayment = async (orderId: string, paymentMethod: OEPPaymentMethod) => {
    try {
      const { error } = await supabase
        .from('oep')                          // ← tabla oep
