...
export const useFullDay = () => {
  ...
  const fetchOrders = useCallback(async (limit = 100) => {
    try {
      ...
      setOrders(data || []);
      console.log('✅ Pedidos FullDay cargados:', data?.length);
    } catch (error) {
      console.error('Error fetching fullday orders:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createOrder = useCallback(async (orderData: {
    ...
    try {
      ...
      console.error('Error creating fullday order:', error);
      return { success: false, error: error.message };
    }
  }, []);

  const updateOrderStatus = useCallback(async (id: string, status: FullDayOrder['status']) => {
    try {
      ...
      console.error('Error updating fullday order status:', error);
      return { success: false, error: error.message };
    }
  }, [orders]);
  ...
  const deleteOrder = useCallback(async (id: string) => {
    try {
      ...
      console.error('Error deleting fullday order:', error);
      return { success: false, error: error.message };
    }
  }, []);
