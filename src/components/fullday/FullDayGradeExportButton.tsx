...
import { exportFullDayGradeExcel, exportFullDayAlfabeticoExcel } from '../../utils/gradeExportUtils';
...
export const FullDayGradeExportButton: React.FC<Props> = ({ orders, selectedDate }) => {
  ...
  const handleExportAlfabetico = async () => {
    ...
    setIsOpen(false);
    try {
      await exportFullDayAlfabeticoExcel(orders, selectedDate);
    } catch (e) {
      console.error('Error exportando FullDay Excel Alfabético:', e);
      alert('Error al exportar. Revisa la consola.');
    } finally {
      setLoading(false);
    }
