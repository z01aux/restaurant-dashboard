...
export const FullDayPaymentModal: React.FC<FullDayPaymentModalProps> = ({
  ...
  const autoCompleteRemaining = (changedField: FieldName) => {
    ...
    if (filledCount === 2) {
      ...
      if (remaining >= 0 && remaining <= total) {
        ...
        if (emptyField) {
          setSplitAmounts(prev => ({ ...prev, [emptyField]: remaining.toFixed(2) }));
        }
      }
    }
  };
  ...
  const isSelected = (value: string) => {
    if (value === 'none') return selectedMethod === null;
          ...
          {showSplitInputs && (
            <div className="mb-5 bg-orange-50 border border-orange-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={handleClearAll} className="text-xs text-orange-600 hover:text-orange-800 underline">Limpiar</button>
              </div>

              {/* Efectivo */}
              <div className="mb-3">
                <label className="text-xs font-medium text-green-700 mb-1 block">💵 Efectivo (S/)</label>
                <input
                  ...
                  placeholder="0.00"
                  disabled={saving}
                  className="w-full px-3 py-2 border-2 border-green-300 rounded-lg text-sm focus:ring-2 focus:ring-green-400 focus:border-green-400 bg-white"
                />
              </div>

              {/* Yape/Plin */}
              <div className="mb-3">
                <label className="text-xs font-medium text-purple-700 mb-1 block">📱 Yape/Plin (S/)</label>
                <input
                  type="text" inputMode="decimal"
                  ...
                  placeholder="0.00"
                  disabled={saving}
                  className="w-full px-3 py-2 border-2 border-purple-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-400 focus:border-purple-400 bg-white"
                />
              </div>

              {/* Tarjeta */}
              <div className="mb-3">
                <label className="text-xs font-medium text-blue-700 mb-1 block">💳 Tarjeta (S/)</label>
                <input
                  type="text" inputMode="decimal"
                  ...
                  placeholder="0.00"
                  disabled={saving}
                  className="w-full px-3 py-2 border-2 border-blue-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-400 focus:border-blue-400 bg-white"
                />
              </div>
              ...
              <div className="bg-white rounded-lg p-3 border border-orange-200 space-y-1">
                <div className="flex justify-between text-xs">
                  ...
                  <span className={`font-semibold ${Math.abs(parseFloat(sumDisplay) - total) < 0.01 ? 'text-green-600' : 'text-orange-600'}`}>
                    S/ {sumDisplay}
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className={`font-semibold ${parseFloat(remaining) === 0 ? 'text-green-600' : parseFloat(remaining) < 0 ? 'text-red-600' : 'text-orange-600'}`}>
                    S/ {remaining}
                  </span>
                </div>
              </div>
              ...
              {splitError && (
                <div className="mt-2 text-xs text-red-600 bg-red-50 rounded-lg p-2 border border-red-200">
                  ⚠️ {splitError}
                </div>
              )}
              <p className="text-xs text-orange-600 mt-2 text-center">
                💡 Completa 2 campos y el tercero se autocompletará
              </p>
            </div>
          )}
