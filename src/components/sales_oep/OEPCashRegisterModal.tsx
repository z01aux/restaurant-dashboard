// ============================================================
// ARCHIVO: src/components/sales_oep/OEPCashRegisterModal.tsx
// Modal de caja para el módulo OEP
// Equivalente exacto de: src/components/sales_fullday/FullDayCashRegisterModal.tsx
// ============================================================
...
interface OEPCashRegisterModalProps {
  isOpen:       boolean;
  onClose:      () => void;
  type:         'open' | 'close';
  cashRegister: any;
...
export const OEPCashRegisterModal: React.FC<OEPCashRegisterModalProps> = ({
  isOpen,
  onClose,
  type,
  cashRegister,
  ...
  useEffect(() => {
    if (isOpen) { setInitialCash(''); setFinalCash(''); setNotes(''); setError(''); }
  }, [isOpen, type]);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {type === 'open' ? 'Abrir Caja OEP' : 'Cerrar Caja OEP'}
            </h2>
        ...
        <div className="p-6">
          {/* Resumen del día (solo al cerrar) */}
          {type === 'close' && (
              ...
              <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="text-2xl font-bold text-blue-800">{todaySummary.totalOrders}</div>
                  </div>
                  <div>
                    <div className="text-sm text-blue-600">Total Ventas</div>
                    <div className="text-2xl font-bold text-blue-800">S/ {todaySummary.totalAmount.toFixed(2)}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    {[
                      { label: 'Efectivo:',   value: todaySummary.efectivo,  color: 'text-green-600'  },
                      { label: 'Yape/Plin:',  value: todaySummary.yapePlin,  color: 'text-purple-600' },
                      { label: 'Tarjeta:',    value: todaySummary.tarjeta,   color: 'text-blue-600'   },
                      { label: 'No Aplica:',  value: todaySummary.noAplica,  color: 'text-gray-600'   },
                    ].map(({ label, value, color }) => (
                      <div key={label} className="flex justify-between text-sm">
                        <span className="text-gray-600">{label}</span>
                        <span className={`font-semibold ${color}`}>S/ {value.toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    <div className="space-y-2">
                      {todaySummary.topProducts.map((p, i) => (
                        <div key={i} className="flex justify-between text-sm">
                          ...
                          <span className="font-semibold text-blue-600">{p.quantity} uds</span>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            {type === 'open' ? (
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Ingresa el dinero con el que empiezas el turno OEP
                </p>
              </div>
            ) : (
              <>
