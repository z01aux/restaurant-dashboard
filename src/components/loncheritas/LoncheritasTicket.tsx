// ============================================
// ARCHIVO: src/components/loncheritas/LoncheritasTicket.tsx
// Ticket Loncheritas con botones Imprimir y PDF (igual que OEP y FullDay)
...
interface LoncheritasTicketProps {
...
const LoncheritasTicket: React.FC<LoncheritasTicketProps> = ({ order, onMouseEnter, onMouseLeave }) => {
    ...
    <Document>
      <Page size={[PAGE_WIDTH]} style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}><Text style={styles.bold}>TIPO:</Text><Text>LONCHERITAS</Text></View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}><Text style={styles.bold}>GRADO:</Text><Text>{order.grade} - Sección {order.section}</Text></View>
          <View style={styles.row}>
  ...
  const generateTicketHTML = (): string => {
    return `
      <div class="ticket">
        ...
        <div class="info-row"><span class="label">TIPO:</span><span class="value">LONCHERITAS</span></div>
        ...
        <div class="center">
          ...
          <div class="normal">*** LONCHERITAS ***</div>
  ...
  const handleDownloadPDF = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const slug = order.student_name.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
      link.href = url;
      link.download = `loncheritas-${order.order_number}-${slug}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
    } catch (error) {
      console.error('Error generando PDF Loncheritas:', error);
    }
  };
