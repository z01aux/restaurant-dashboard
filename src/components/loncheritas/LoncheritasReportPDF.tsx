// ============================================
// ARCHIVO: src/components/loncheritas/LoncheritasReportPDF.tsx
...
// ─────────────────────────────────────────
// PALETA — naranja/ámbar (colores Loncheritas)
// ─────────────────────────────────────────
const C = {
  primary:      '#F97316',
  primaryDark:  '#C2410C',
...
interface LoncheritasReportData {
  period: string;
  generatedAt: string;
  totalOrders: number;
  totalAmount: number;
...
// DOCUMENTO PDF
// ─────────────────────────────────────────
const LoncheritasReportDocument: React.FC<{ data: LoncheritasReportData }> = ({ data }) => {
  ...
  return (
    <Document title={`Loncheritas — ${data.period}`} author="Mary's Restaurant">
      <Page size="A4" style={S.page}>
        ...
        <View style={S.header}>
          ...
          <View style={S.headerRight}>
            <Text style={S.reportTitle}>Reporte Loncheritas</Text>
        ...
        <View style={S.footer} fixed>
          ...
          <Text style={S.footerBrand}>MARY'S RESTAURANT — Loncheritas</Text>
...
// BUILDER DE DATOS
// ─────────────────────────────────────────
const buildLoncheritasReportData = (
  orders: LoncheritasOrder[],
  startDate: Date,
  endDate: Date,
): LoncheritasReportData => {
...
// FUNCIÓN PRINCIPAL
// ─────────────────────────────────────────
export const generateLoncheritasReportPDF = async (
  orders:    LoncheritasOrder[],
  startDate: Date,
  endDate:   Date,
): Promise<void> => {
  ...
  const reportData = buildLoncheritasReportData(filtered, startDate, endDate);
  const blob = await pdf(<LoncheritasReportDocument data={reportData} />).toBlob();

  const startStr = startDate.toISOString().split('T')[0].replace(/-/g, '');
  const endStr   = endDate.toISOString().split('T')[0].replace(/-/g, '');
  const fileName = `loncheritas_reporte_${startStr}_al_${endStr}.pdf`;
  ...
  console.log('✅ PDF Loncheritas generado:', fileName);
};

export default LoncheritasReportDocument;