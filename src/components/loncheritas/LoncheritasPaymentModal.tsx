...
// ✅ ACTUALIZADO: CON PAGO MIXTO Y AUTOCOMPLETADO
// =================================================================

import React, { useState, useEffect } from 'react';
import { X, CreditCard, DollarSign, Smartphone, Minus, PieChart } from 'lucide-react';
...
export const LoncheritasPaymentModal: React.FC<LoncheritasPaymentModalProps> = ({
  ...
  const [selectedMethod, setSelectedMethod] = useState<LoncheritasPaymentMethod>(null);
  ...
  useEffect(() => {
    if (order) {
      ...
      if (order.split_payment) {
        setSplitAmounts({
          efectivo: order.split_payment.efectivo.toString(),
          yapePlin: order.split_payment.yapePlin.toString(),
          tarjeta:  order.split_payment.tarjeta.toString(),
        });
      } else {
  ...
  const getFilledFieldsCount = () => {
    ...
    return [v.efectivo > 0, v.yapePlin > 0, v.tarjeta > 0].filter(Boolean).length;
  };

  const autoCompleteRemaining = (changedField: FieldName) => {
    const values = getNumericValues();
    if (getFilledFieldsCount() === 2) {
      const remaining = total - (values.efectivo + values.yapePlin + values.tarjeta);
      if (remaining >= 0 && remaining <= total) {
        const fields: FieldName[] = ['efectivo', 'yapePlin', 'tarjeta'];
        const emptyField =
          fields.find(f => f !== changedField && values[f] === 0) ||
  ...
  const handleSelectMethod = (value: string) => {
    ...
    else if (value === 'none') { setSelectedMethod(null); setShowSplitInputs(false); }
    else { setSelectedMethod(value as LoncheritasPaymentMethod); setShowSplitInputs(false); }
  };
  ...
  const paymentOptions = [
    ...
    { value: 'none',      label: 'No Aplica',  icon: Minus,      bgSel: 'bg-gray-100',  borderSel: 'border-gray-400',   textSel: 'text-gray-600',   hoverBorder: 'hover:border-gray-300'   },
  ];

  const isSelected = (value: string) => value === 'none' ? selectedMethod === null : selectedMethod === value;
  ...
  // Color del header — Loncheritas usa verde esmeralda
  const headerGradient = 'bg-gradient-to-r from-emerald-600 to-teal-600';
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className={`${headerGradient} p-4 sm:p-5 text-white sticky top-0 z-10`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-100 mt-0.5">Pedido #{order.order_number} · 🍱 Loncheritas</p>
              </div>
            </div>
        ...
        <div className="p-5">
          ...
          <div className="bg-gray-50 rounded-xl p-4 mb-5 border border-gray-200">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-xl font-bold text-emerald-600">S/ {total.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="mb-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {paymentOptions.slice(3).map(opt => {
                const Icon = opt.icon;
                const sel  = isSelected(opt.value);
                return (
                  <button key={opt.value} type="button" onClick={() => handleSelectMethod(opt.value)} disabled={saving} className={getOptionClasses(opt)}>
          ...
          {showSplitInputs && (
            <div className="mb-5 bg-orange-50 border border-orange-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => { setSplitAmounts({ efectivo: '', yapePlin: '', tarjeta: '' }); setSplitError(null); }} className="text-xs text-orange-600 hover:text-orange-800 underline">Limpiar</button>
              </div>
              {[
                { field: 'efectivo' as FieldName, label: '💵 Efectivo (S/)', border: 'border-green-300', focus: 'focus:ring-green-400 focus:border-green-400', labelColor: 'text-green-700' },
                { field: 'yapePlin' as FieldName, label: '📱 Yape/Plin (S/)', border: 'border-purple-300', focus: 'focus:ring-purple-400 focus:border-purple-400', labelColor: 'text-purple-700' },
                { field: 'tarjeta'  as FieldName, label: '💳 Tarjeta (S/)',  border: 'border-blue-300',   focus: 'focus:ring-blue-400 focus:border-blue-400',     labelColor: 'text-blue-700'   },
              ].map(({ field, label, border, focus, labelColor }) => (
                <div className="mb-3" key={field}>
                  <label className={`text-xs font-medium ${labelColor} mb-1 block`}>{label}</label>
                  <input
                    type="text" inputMode="decimal"
                    value={splitAmounts[field]}
                    onChange={e => handleSplitAmountChange(field, e.target.value)}
                    onBlur={() => handleBlur(field)}
                    placeholder="0.00" disabled={saving}
                    className={`w-full px-3 py-2 border-2 ${border} rounded-lg text-sm ${focus} bg-white`}
                  />
                </div>
              ))}
          ...
          <div className="flex space-x-3">
            ...
            <button type="button" onClick={handleSave} disabled={saving || (selectedMethod === order.payment_method && selectedMethod !== 'MIXTO')}
              className={`flex-1 ${headerGradient} text-white px-4 py-3 rounded-xl hover:shadow-lg transition-all duration-300 disabled:opacity-50 flex items-center justify-center space-x-2 font-semibold`}>
              {saving ? (
