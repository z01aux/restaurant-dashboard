...
interface LoncheritasPaymentModalProps {
  ...
  onSave: (orderId: string, paymentMethod: LoncheritasPaymentMethod, splitDetails?: LoncheritasSplitPaymentDetails) => Promise<void>;
  onPaymentUpdated?: (orderId: string, newMethod: LoncheritasPaymentMethod, splitDetails?: LoncheritasSplitPaymentDetails) => void;
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4" style={{backgroundColor:'rgba(0,0,0,0.5)'}} onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e=>e.stopPropagation()}>
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-4 sm:p-5 text-white sticky top-0 z-10">
        ...
        <div className="p-5">
          ...
          <div className="flex space-x-3">
            ...
            <button type="button" onClick={handleSave} disabled={saving||(!selectedMethod&&selectedMethod!==order.payment_method)}
              className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-4 py-3 rounded-xl hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center space-x-2 font-semibold">
