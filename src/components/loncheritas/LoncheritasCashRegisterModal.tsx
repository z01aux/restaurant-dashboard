// ============================================
// ARCHIVO: src/components/loncheritas/LoncheritasCashRegisterModal.tsx
// ============================================
...
interface LoncheritasCashRegisterModalProps {
    isOpen: boolean;
    onClose: () => void;
    type: 'open' | 'close';
    cashRegister: any;
...
export const LoncheritasCashRegisterModal: React.FC<LoncheritasCashRegisterModalProps> = ({
    isOpen,
    onClose,
    type,
    cashRegister,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (type === 'open') {
            ...
            if (isNaN(cash) || cash < 0) { setError('Ingresa un monto inicial válido'); return; }
            await onConfirm({ initialCash: cash });
        } else {
            const cash = parseFloat(finalCash);
            if (isNaN(cash) || cash < 0) { setError('Ingresa un monto final válido'); return; }
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                {/* Header — diseño de Órdenes */}
                <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex justify-between items-center">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">
                            {type === 'open' ? '🔓 Abrir Caja Loncheritas' : '🔒 Cerrar Caja Loncheritas'}
                        </h2>
                ...
                <div className="p-6">
                    ...
                    {/* Resumen del día — solo al cerrar, diseño de FullDay */}
                    {type === 'close' && (
                        <div className="mb-6 space-y-4">
                            <div className="bg-orange-50 rounded-xl p-4 border border-orange-200">
                                <h3 className="font-semibold text-orange-800 mb-3 flex items-center">
                                    <Clock size={18} className="mr-2" />
                                    Resumen del Día - Loncheritas
                                </h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <div className="text-sm text-orange-600">Total Pedidos</div>
                                        <div className="text-2xl font-bold text-orange-800">{todaySummary.totalOrders}</div>
                                    </div>
                                    <div>
                                        <div className="text-sm text-orange-600">Total Ventas</div>
                                        <div className="text-2xl font-bold text-orange-800">S/ {todaySummary.totalAmount.toFixed(2)}</div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div className="space-y-2">
                                            {todaySummary.topProducts.map((p, i) => (
                                                <div key={i} className="flex justify-between text-sm">
                                                    ...
                                                    <span className="font-semibold text-orange-600">{p.quantity} uds</span>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="0.00" required disabled={loading} autoFocus
                                    />
                                </div>
                                <p className="text-xs text-gray-500 mt-1">Ingresa el dinero con el que empiezas el turno Loncheritas</p>
                            </div>
                        ) : (
                            <>
                                <div>
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        placeholder="Ej: Diferencia en caja, incidencias, etc." disabled={loading}
                                    />
                                </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="border-t border-gray-200 my-2 pt-2">
                                            <div className="flex justify-between font-bold">
                                                ...
                                                <span className="text-orange-600">S/ {expectedTotal.toFixed(2)}</span>
                                            </div>
                                        </div>
                                        {difference !== null && (
                        ...
                        <div className="flex space-x-3 pt-4">
                            ...
                            <button type="submit" disabled={loading}
                                className="flex-1 bg-gradient-to-r from-orange-500 to-amber-500 text-white px-4 py-3 rounded-lg hover:shadow-md transition-all duration-300 disabled:opacity-50 flex items-center justify-center space-x-2 font-semibold">
                                {loading ? (
                                    <>
