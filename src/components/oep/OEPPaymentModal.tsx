...
interface OEPPaymentModalProps {
  ...
  onPaymentUpdated?: (orderId: string, newMethod: OEPPaymentMethod | null) => void;
}
...
export const OEPPaymentModal: React.FC<OEPPaymentModalProps> = ({
  ...
  const [selectedMethod, setSelectedMethod] = useState<OEPPaymentMethod | null>(null);
  ...
  const handleSave = async () => {
    if (selectedMethod === order.payment_method && selectedMethod !== 'MIXTO') { onClose(); return; }
    setLoading(true);
    try {
      await onSave(
        order.id,
  ...
  const handleSelect = (value: string) => {
    if (value === 'none') {
      ...
      setSelectedMethod(value as OEPPaymentMethod);
      ...
      >
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex space-x-3 pt-2">
            ...
            <button
              onClick={handleSave}
              disabled={loading || !isMixtoValid}
              className="flex-1 bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-4 py-3 rounded-xl hover:shadow-lg transition-all duration-300 disabled:opacity-50 flex items-center justify-center space-x-2 font-semibold"
            >
              {loading ? (
