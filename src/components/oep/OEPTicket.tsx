...
const OEPTicket: React.FC<OEPTicketProps> = ({ order }) => {
    ...
    return `
      <div class="ticket">
        ...
        <div class="info-row">
          ...
          <span class="customer-name-bold">${customerDisplay.toUpperCase()}</span>
        </div>
        ${order.phone ? `
        <div class="info-row">
  ...
  const handleDownloadPDF = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const slug = order.customer_name
        .toLowerCase()
        .replace(/[^a-z0-9]/g, '-')
        .replace(/-+/g, '-')
        .replace(/^-|-$/g, '');
