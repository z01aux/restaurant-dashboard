...
export const CashRegisterModal: React.FC<CashRegisterModalProps> = ({
  ...
  // Resumen del día calculado desde el todaySummary recibido
  const summaryData = useMemo(() => {
    if (!todaySummary) return null;
    
    return {
      totalOrders: todaySummary.total_orders,
      totalAmount: todaySummary.total_amount,
      efectivo: todaySummary.by_payment_method.EFECTIVO,
      yapePlin: todaySummary.by_payment_method.YAPE_PLIN,
      tarjeta: todaySummary.by_payment_method.TARJETA,
      noAplica: todaySummary.by_payment_method.NO_APLICA,
      delivery: todaySummary.by_order_type.delivery,
      walkIn: todaySummary.by_order_type.walk_in,
      phone: todaySummary.by_order_type.phone,
    };
  }, [todaySummary]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {/* Resumen del día - solo al cerrar - diseño moderno */}
          {type === 'close' && summaryData && (
            <div className="mb-6 space-y-4">
              ...
              <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-2xl font-bold text-blue-800">
                      S/ {summaryData.totalAmount.toFixed(2)}
                    </div>
                  </div>
                </div>
              </div>

              {/* Grid de dos columnas con métodos de pago y tipos de pedido */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-green-600">
                        S/ {summaryData.efectivo.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Yape/Plin:</span>
                      <span className="font-semibold text-purple-600">
                        S/ {summaryData.yapePlin.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Tarjeta:</span>
                      <span className="font-semibold text-blue-600">
                        S/ {summaryData.tarjeta.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">No Aplica:</span>
                      <span className="font-semibold text-gray-600">
                        S/ {summaryData.noAplica.toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <h4 className="font-medium text-gray-700 mb-3 flex items-center">
                    <span className="mr-2">📋</span> Tipo de Pedido
                  </h4>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Delivery:</span>
                      <span className="font-semibold text-orange-600">
                        S/ {summaryData.delivery.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Local:</span>
                      <span className="font-semibold text-green-600">
                        S/ {summaryData.walkIn.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Cocina:</span>
                      <span className="font-semibold text-blue-600">
                        S/ {summaryData.phone.toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {type === 'open' ? (
              ...
              <>
                ...
                {/* Diferencia esperada - diseño moderno */}
                {summaryData && cashRegister && (
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <h4 className="font-medium text-gray-700 mb-2 flex items-center">
                      <span className="mr-2">📊</span> Diferencia Esperada
                    </h4>
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-600">Monto inicial:</span>
                        <span className="font-semibold">
                          S/ {cashRegister.initial_cash?.toFixed(2) || '0.00'}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
